...
its review status, and saves the result in JSON Lines format.
...
logger = setup_logging("preprocessing", "normalize_advisory")
...
def normalize_advisory(advisory_json):
	...
	delta_nvd = nvd_published_at_parsed - published_at_parsed if nvd_published_at else None
	time_to_nvd = ceil(delta_nvd.total_seconds() / 86400) if delta_nvd else None

	# Extract year and month for cyclical encoding
	...
	delta = github_reviewed_at_parsed - published_at_parsed if github_reviewed_at else None
	time_to_review = ceil(delta.total_seconds() / 86400) if github_reviewed_at else None
...
def run():
	...
	output_file_json = output_folder / "normalized_advisory.json"
